...
//const int zm_speed = 3;//僵尸行走速度
//const int sunshine_product_speed = 150;//向日葵生产阳光的速率
//const int sunshine_collect_speed = 0.15;//阳光收集的速率，由于收集依赖于贝塞尔曲线实现，故此处的取值范围应该为0~1，越大收集越快
//使用全局常量会出问题，这里就直接使用宏定义来进行替换了
#define zm_speed 3	//僵尸行走速度
#define sunshine_product_speed 150//向日葵生产阳光的速率
#define sunshine_collect_speed 0.15//阳光收集的速率，由于收集依赖于贝塞尔曲线实现，故此处的取值范围应该为0~1，越大收集越快
#define wandou_bullet_product_speed 50//豌豆子弹的生产速率，越大越慢
...
extern IMAGE *imgloadacount[4];//系统的登录(没时间写了，套个皮)
...
extern struct zm zms[zm_MAX];	//僵尸池,用来事先存储僵尸
...
extern struct zhiWu plant_map[3][9];	//地图数组，方便存储植物
